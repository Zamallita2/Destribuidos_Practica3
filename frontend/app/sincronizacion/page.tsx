...
function formatTime(value?: string) {
  return value ? new Date(value).toLocaleTimeString("es-BO") : "—";
}

function count(value: number | null | undefined) {
  return value == null ? "Sin lectura" : value.toLocaleString("es-BO");
}
  ...
  return <div className="space-y-6 text-white">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-gray-300">
        <RefreshCw className="mr-2 inline h-3 w-3" />Actualización automática · última lectura {formatTime(status?.observed_at)}
      </div>
    </div>
      ...
      {nodeKeys.map((key) => {
        ...
        return <section key={key} className="rounded-xl border border-white/10 bg-[#202530] p-5">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div><p className="text-xs text-gray-400">Vuelos</p><p className="text-xl font-bold">{count(node?.flights)}</p></div>
            <div><p className="text-xs text-gray-400">Boletos</p><p className="text-xl font-bold">{count(node?.tickets)}</p></div>
          </div>
          <p className="mt-4 text-xs text-gray-400">{online ? `Responde en ${node.latency_ms} ms` : `Última vez activo: ${formatTime(node?.last_seen_at)}`}</p>
          {key !== "mongo" && <p className="mt-1 text-xs text-gray-400">Cola pendiente: <strong className="text-white">{count(node?.pending)}</strong></p>}
        </section>;
      })}
    </div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="rounded-xl border border-white/10 bg-[#202530] p-5">
        ...
        <div className="mt-4 space-y-3 text-sm">
          <div className="flex justify-between"><span>Vuelos AM ↔ EU</span><strong>{count(status?.count_difference.am_eu_flights)}</strong></div>
          <div className="flex justify-between"><span>Boletos AM ↔ EU</span><strong>{count(status?.count_difference.am_eu_tickets)}</strong></div>
          <div className="flex justify-between"><span>Vuelos AM ↔ Mongo</span><strong>{count(status?.count_difference.am_mongo_flights)}</strong></div>
          <div className="flex justify-between"><span>Vuelos EU ↔ Mongo</span><strong>{count(status?.count_difference.eu_mongo_flights)}</strong></div>
        </div>
        <div className="mt-5 rounded-lg bg-blue-500/10 p-3 text-sm">Operaciones esperando entrega completa: <strong>{count(status?.pending)}</strong></div>
      ...
      <section className="rounded-xl border border-white/10 bg-[#202530] p-5">
        ...
        <ol className="mt-4 max-h-80 space-y-3 overflow-y-auto">
          {[...(status?.events ?? [])].reverse().map((event) => <li key={event.id} className="border-l-2 border-blue-500/50 pl-3 text-sm"><time className="text-xs text-gray-400">{formatTime(event.at)}</time><p>{event.message}</p></li>)}
